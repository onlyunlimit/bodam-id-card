...
| `portal.html` | 서울 지도 관제, 게이트 분포, 부서 상태, 날씨·시각, 수신 브리핑 |
| `departments.html` | 오리진·비콘·실드 조직 안내 |
| `origin.html`, `beacon.html`, `shield.html` | 부서별 테마·요원 카드·시간표·접이식 메신저 |
| `entertainment.html`, `lucky.html`, `obsidus.html` | 헌터 기획사와 크루·팬 채널 |
| `orpe.html` | 에이지 기밀 프로필·동선 추적·감청 로그 |
| `records.html` | 모의 신고·수신 주파수·영문 종결 도장 |
| `headquarters.html` | 지상 8층·지하 10층 단면도와 부서 위치 |
| `manual.html` | 로어북 기반 세계관·게이트·등급·가이딩·매칭·명령어 안내 |
| `community.html` | 팬 게시판·SGIA 익명 게시판·작성자 수정/삭제 |
...
- 카드 클릭·키보드·스와이프로 앞뒤 회전, 0.8초 길게 누르면 캐릭터로 이동합니다. 여명은 COMING SOON입니다. `image.md` 원본 자체를 읽거나 배포하지 않습니다.
...
- 생성 이미지로 건물·배경을 사용하지 않습니다. 작은 사원증 프레임 텍스처만 `assets/art/id-frame.png`에 보관하고 CSS 마스크로 가장자리에 적용합니다. 로고와 UI 아이콘·구조도는 SVG/CSS로 만듭니다.
