...
import { activityFor, koreaTime, scheduleFor, generateIncident, isIncident, tally, teams } from '../assets/model.js';
import { characters } from '../assets/characters.js';

test('KST date and time cross UTC midnight independently of local timezone', () => {
  assert.deepEqual(koreaTime(new Date('2026-09-15T15:00:00Z')), {date:'2026.09.16',clock:'00:00:00',minutes:0});
...
test('Beacon shifts respect exact start, lunch and end boundaries', () => {
  for(const [time,label,active] of [[539,'업무 외 시간',false],[540,'타겟 정보 브리핑',true],[719,'정보 조사 · 외근',true],[720,'점심 식사',false],[779,'점심 식사',false],[780,'추적 · 대상 접촉',true],[1050,'일일 보고서 작성',true],[1110,'업무 외 시간',false]]) {
    const actual=activityFor('beacon',time);assert.equal(actual.label,label);assert.equal(actual.active,active);
  }
});
...
test('No source schedule overlaps or impossible active intervals', () => {
  for(const team of teams) {
    let previous=0;
    for(const [start,end] of scheduleFor(team.id)) {assert.ok(start>=previous);assert.ok(end>start&&end<=1440);previous=end;}
  }
});
test('All five incident categories tally, resolve and reopen consistently', () => {
  const records=Array.from({length:5},(_,i)=>generateIncident(i,()=>.5,123456789));
  ...
  records[0].resolvedAt=null;assert.equal(tally(records).gates,1);
