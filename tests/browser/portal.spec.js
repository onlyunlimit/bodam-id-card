...
test('Staff incident stamps reconcile counters, persist and can be undone',async({page})=>{
  await page.goto('/');await page.locator('#enter-staff').click();
...
test('Public server never serves originals, personal images, or repository metadata',async({request})=>{
  ...
  for(const pathname of ['/monster.html','/sgia.html','/team.html']){const response=await request.get(pathname);expect(response.status()).toBe(200);expect(await response.text()).toContain('index.html#resources');}
});
test('Horizontal drag flips, cancelled hold does not navigate, long press connects',async({page})=>{
  ...
  expect(new URL(page.url()).pathname).toBe('/');await expect(card).toHaveAttribute('aria-pressed','true');
...
test('Weather failure, corrupt storage, sound and terminal styles remain usable',async({page})=>{
  ...
  await page.goto('/');await expect(page.locator('#weather-status')).toContainText('기상 연결 대기');
...
test('Mobile layout, touch flip, reduced motion and portrait failure fallback',async({page})=>{
  ...
  await page.goto('/');await expect(page.locator('html')).toHaveAttribute('data-motion','off');
